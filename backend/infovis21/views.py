...
def artists():
    ...
    d = {}
    pipeline = [
        {"$project": {"text": "$name", "value": "$popularity", "_id": 0}},
    ]
...
def _graph():
    ...
    if not (_x and _y and _zoom and dimx and dimy and d["type"]):
        return abort(
            400,
            description="Please specify x and y coordinates, type, zoom level and x and y dimensions, e.g. /graph?x=200&y=200&zoom=0&dimx=acousticness&dimy=loudness&type=genre",
        )
