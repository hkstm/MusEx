...
class MusicHeatmap extends Component<{}, any> {
  ...
  componentDidUpdate(prevProps: any, prevState: any) {
    if (this.state.yearsCount !== prevState.yearsCount) this.getData();
  }

  getData() {
    // setTimeout(this.d3heatmap, 0);
    axios
      .get(`http://localhost:5000/v1/years?limit=${this.state.yearsCount}`)
      .then((res) => {
    ...
    return (
      <div className="heatmap-grid">
        ...
        <div className="heatmap-row">
          <div className="heatmap-tick-y" />
          {this.state.x.map((year: any, i: number) => (
            <div key={year} className="heatmap-tick">
              {" "}
              {year}{" "}
            </div>
          ))}
